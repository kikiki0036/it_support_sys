...
function ConfirmationDialogRaw(props) {
  ...
  const getData = async () => {
      try {
          await axios.get('http://localhost:5000/getLoaction', {  }).then((res) => {  
            setData(res.data);
          })
      } catch (error) {
          console.log(error);
    ...
    >
      <DialogTitle sx={{  p: 1.7 , color:"#1976d2", displa:"flex",alignItems:"center"}} >
        ...
        <Paper component="form" sx={{ boxShadow: 0, mb:1, p: '1px 4px', display: 'flex', alignItems: 'center', width: "100%" }}>
            ...
            <InputBase sx={{ ml: 1, flex: 1,px:0,fontSize: '0.75rem' }} onChange={ (e) => { onSearch(e.target.value) } } placeholder="Search location" inputProps={{ 'aria-label': 'search google maps' }} />
          ...
          <div className="dialog-box-input dialogContent">
                ...
                <table>
                  ...
                  <tbody className="dialog-table-body-h">
                      {
                        rows.map((item, index) => (
                            <tr key={index} onClick={() => { handleClose( {id:item.location_id, name:item.location}) }}>
                              <td>{item.location}</td>
                            </tr>
                        ))
                      }
...
const RenderDataLocation = (props) => {
...
export default RenderDataLocation
