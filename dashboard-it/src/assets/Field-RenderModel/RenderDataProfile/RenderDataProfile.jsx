...
function ConfirmationDialogRaw(props) {
  ...
  const getData = async () => {
      try {
          await axios.get('http://localhost:5000/getEmpProfile', {  }).then((res) => {  
            setData(res.data);
          })
      } catch (error) {
          console.log(error);
          ...
          <div className="dialog-box-input dialogContent">
                ...
                <table>
                  ...
                  <tbody className="dialog-table-body-h">
                      {
                        rows.map((item, index) => (
                            <tr key={index} onClick={() => { handleClose( {id:item.id_profile, name:item.profile_name}) }}>
                              <td>{item.id_emp}</td>
                              <td className="font-family-Kanit">{item.data_emps[0].emp_name}</td>
                              <td className="font-kanit">{item.profile_name}</td>
                              <td className="transform-none-txt">{item.email}</td>
                            </tr>
                        ))
                      }
                  </tbody>
...
export default RenderDataProfile
